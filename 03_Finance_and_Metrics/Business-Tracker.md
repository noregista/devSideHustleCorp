...
| ⬜ 未着手 | `/simulator` 768px(タブレット縦)でのUI窮屈さ改善 | 任意 | オーナーレビュー(2026-06-11)で768×900の通常UI表示は窮屈と指摘。クラッシュ修正のブロッカーではないため今回は未対応。改善候補: ①フォールバック対象を1024px未満に拡大する ②左/右パネルをDrawer化(開閉式)して常時表示しない |
