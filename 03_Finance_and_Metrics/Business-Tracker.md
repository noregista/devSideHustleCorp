...
| ⚠️ リリース前必須(事前準備完了・本検証は未実施) | **Keepa API検証**（Amazon.co.jp 家具40件/10カテゴリ） | リリース前 | 検証スクリプト・対象リスト・結果フォーマット(JSON/CSV/MD)・判定基準は整備済み(2026-06-12)。`verify-keepa`は`not_run`(`KEEPA_API_KEY`未設定時・デフォルト=実APIを呼ばず安全終了)/`demo`(`--demo`、`source: mock_demo`・本番採用判断には使用不可)/`real`(キー設定時)の3モード。実行順序: ①`scripts/fixtures/keepa-targets.json`のASIN実在確認 → ②€19/月課金(オーナー判断待ち) → ③`KEEPA_API_KEY`設定 → ④`pnpm --filter @fls/scripts verify-keepa`実行。詳細: `docs/keepa-verification-targets.md` |
...
- 今すぐやらない理由: 「独自性」は主観的判断要素が大きく、方向性についてオーナーの好み・ブランドイメージ確認がコード変更前に必要。ドキュメント整理フェーズでは着手しない
- 着手タイミング: 公開後、もしくは公開前でもオーナーとデザイン方向性(イラスト追加可否・トンマナ)を確認できたタイミング
