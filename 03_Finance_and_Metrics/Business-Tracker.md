...
| ⚠️ オーナー判断待ち(2026-06-13) | **Keepa API検証の位置づけ決定（案A/案B）** | 公開判断の前提 | **案A**: 「公開前必須」を維持(従来方針、下記3行のまま)。**案B**: 「URL自動取得機能のリリース前必須」へ格下げし、手動入力版で先行公開(既存`NEXT_PUBLIC_ENABLE_URL_IMPORT`デフォルトにより本番で「+URL」は「準備中」表示のまま＝MockProviderのダミーデータは表示されない)。案Bの場合、下記3行(Keepa API検証/採用判定/MockProviderフォールバック)は公開後(フェーズ2)タスクへ降格。詳細は`docs/release-checklist.md`セクション7・14参照 |
| ⚠️ 位置づけ2案・上記の選択待ち(事前準備完了・本検証は未実施) | **Keepa API検証**（Amazon.co.jp 家具40件/10カテゴリ） | 案Aなら公開前／案Bなら公開後(フェーズ2) | 検証スクリプト・対象リスト・結果フォーマット(JSON/CSV/MD)・判定基準は整備済み(2026-06-12)。`verify-keepa`は`not_run`(`KEEPA_API_KEY`未設定時・デフォルト=実APIを呼ばず安全終了)/`demo`(`--demo`、`source: mock_demo`・本番採用判断には使用不可)/`real`(キー設定時)の3モード。実行順序: ①`scripts/fixtures/keepa-targets.json`のASIN実在確認 → ②€19/月課金(オーナー判断待ち) → ③`KEEPA_API_KEY`設定 → ④`pnpm --filter @fls/scripts verify-keepa`実行。詳細: `docs/keepa-verification-targets.md` |
| ⚠️ 位置づけ2案・上記の選択待ち | Keepa採用判定・KeepaProvider 本番切替 | 案Aなら公開前／案Bなら公開後(フェーズ2) | 検証結果に基づき判定。寸法が取れなくても商品名+画像URLだけで価値あり → 手動修正UIにフォールバック |
| ⚠️ 位置づけ2案・上記の選択待ち | MockProviderのモック商品フォールバック方針決定 | 案Aなら公開前／案Bなら解消済み | KEEPA_API_KEY未設定時、実URLでも「モック商品(ASIN)」+疑似乱数寸法が「要確認」バッジのみで配置可能になる問題。**案B**の場合、本番で`NEXT_PUBLIC_ENABLE_URL_IMPORT`を`true`にしない(既存デフォルト)限り「+URL」自体が無効のため実害なし＝対応不要。release-checklist.md参照 |
...
| ✅ ドラフト完了 | 利用規約・プライバシーポリシー・免責文言ページ | リリース前にオーナー最終確認 | `/terms`・`/privacy`を新設、LP/`/simulator`/`/share`に導線・免責一文を追加。運営者名・連絡先・管轄裁判所等は仮表記。release-checklist.md セクション10参照 |
| ✅ 完了(無効状態) | GA4導入（8イベント計測基盤） | 2026-06-11 | `NEXT_PUBLIC_GA_MEASUREMENT_ID`未設定時はスクリプト未読込・無効。設計8イベント実装済み・type-check pass・E2E 31/31 pass。**本番有効化時は`/privacy`更新が必須**(release-checklist.md セクション5参照) |
