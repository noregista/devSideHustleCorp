...
**[完了] 「採寸ツール感」解消のためのUI/UXポリッシュ第2弾(優先度A1-A8)が無エラーで一括完成**
- 直前のオーナーレビューで「採寸ツール感」「AIテンプレ感」「開発中ツール感」を指摘され、8項目(寸法ラベルの選択時限定表示・人物配色/比率調整・3D家具の面別シェーディング・共有ページ文言改善・価格未設定表示の弱体化+インライン価格入力導線・LPヒーローのカード化・人のスケールカードの優先度低下・右パネル空状態の3カードガイド化)を実装
- 既存の`self-check-ui-review.ts`を雛形に新スクリプト`self-check-ui-priority-a-after.ts`を作成し、LP/2D(未選択・選択・人物配置)/3D/共有(PC・モバイル)/インテリアリストの10枚を一括撮影する流れを確立。インテリアリストの撮影では「価格を追加」ボタンをクリック→金額入力→Enterで`updateProduct`を発火させ、新しい価格入力UIごと検証できた
- type-check・E2E 31/31・preflightはいずれも一発PASS(修正0件)。寸法ラベルを`isSelected &&`で囲むだけ、配色は定数3つの変更のみ、空状態は既存`data-testid`を保てば自由に再構成できる、といった「最小差分で見た目の印象を大きく変えられる」設計だったことが、後工程の手戻りゼロにつながった
- 教訓: 「機能を変えずに印象を変える」系のレビュー対応は、(1)表示条件の追加(`isSelected &&`等)、(2)色・比率定数の置き換え、(3)コピー文言とコンポーネント構造の置き換え、の3パターンに分解できることが多く、E2Eが`data-testid`ベースで書かれていれば視覚的な大幅変更でも回帰リスクが低い。今後も「テスト属性は保持し、内部のJSX/文言/数値だけ差し替える」方針を継続する

**[決定] UI改善フェーズをクローズし、Keepa検証は保留のまま優先度B/Cバックログ整理に着手**
- オーナーから優先度A全8項目(commit `c6243e7`、push済み)のスクリーンショットレビューを受け、「採寸図感の解消」「公開前の見た目の違和感低減」という目的は達成と判断され、本UI改善フェーズはpush後クローズとなった
- Keepa API検証は課金判断が必要なため、オーナー判断が出るまで着手しない方針を再確認(Keepa以外の自動で進められる整理作業を先に進める)
- 次に進めるべき「自動で進められる」作業として、残課題8件(2D家具質感・3D家具表現・人物配置/重なり順・LP独自性・共有ページ・モバイルフォールバック・インテリアリスト価格表示・3D壁床ラグ質感)を優先度B/Cでバックログ化し、`Business-Tracker.md`に「UI/デザイン改善バックログ（優先度B/C・公開後改善候補）」として整理。各項目に目的・現状の問題・実装方針・今すぐやらない理由・着手タイミングを記載した
- `docs/release-checklist.md`にセクション14「公開判断マトリクスとUI/デザイン改善バックログ」を新設し、「公開ブロッカー」「公開前にできれば(任意)」「公開後改善でよい」「オーナー判断が必要」「自動で進められる」の5分類で残作業全体を整理。**見た目の改善(B1-C5)と公開ブロッカー(Keepa・MockProvider方針・デプロイ先/ドメイン・利用規約等のプレースホルダー確定)は完全に別軸**であり、見た目の改善が未着手でも公開ブロッカーが解消すれば公開可能、という整理を明文化した
- 教訓: 「見た目を磨く」と「公開できるかどうか」は混同しやすいが、別の軸として管理すると、オーナー判断待ちの項目(Keepa等)がある間も「自動で進められる」見た目改善のバックログ整理・優先度付けという形で前進できる。今後、新しい改善要望が出た際は、まず「公開ブロッカーか/公開後でよいか」「オーナー判断が必要か/自動で進められるか」の2軸で即座に分類する運用を継続する

**[実装パターン] 配置物どうしの重なり判定は「中心点が相手のAABB内か」ではなく「自分のフットプリントAABBと相手のAABBの矩形重なり」で行う**
- B2(人物の初期配置・重なり順改善)で、`findFreeStandingPositionCm`の重なり判定が「人物の中心点が家具AABB内かどうか」のみだったため、人物のフットプリント(45×25cm等)の端が家具や他の人物に食い込んでいても中心点だけが空いていれば「配置可能」と誤判定されていた
- → `getRotatedAABB`(回転考慮済みの中心+半径)と`aabbOverlap`(各軸の中心間距離 < 半径の和)を新設し、`overlapsAnyRect`で候補位置のフットプリントAABBと家具・他人物のAABB全てとの矩形重なりを判定する方式に変更。`findFreeStandingPositionCm`の判定対象も家具のみ→家具+配置済み人物の両方に拡張した
- 教訓: 配置物が点ではなくサイズ(フットプリント)を持つ以上、重なり判定は最初からAABB-AABB判定で実装すべき。中心点判定は一見動いているように見えても、サイズの大きいオブジェクトと小さいオブジェクトの組み合わせで簡単に破綻する

**[設計原則] 配置データの並び順(保存・共有データ)と画面上の描画順(z-order)は分離して管理する**
- B2で「選択中の人物を常に最前面に表示したい」という要件があったが、`people`配列そのものの順序を変えると保存データ・共有データ・リミックス時の順序にも影響してしまう
- → `RoomCanvas2D`では`people`配列はそのまま保存・共有用に保持し、選択中の要素だけを末尾に回した描画専用の`renderedPeople`を別途生成してそちらをmapする。「データの正規の順序」と「その瞬間の描画順」を常に別物として扱った
- 教訓: 表示上の優先順位(最前面表示・ハイライト等)が必要になったら、まず元データを並べ替えずに描画用の派生配列で対応できないか検討する。データの順序変更は保存・共有・他機能への影響範囲が広く、表示専用の問題に対しては割に合わないことが多い

**[方針確認] 共有ページの視認性向上は「本体の不透明度」で対応し、寸法ラベルを強めると「採寸図感」が再発する**
- 2026-06-12のUI/UXポリッシュ第2弾で「採寸図感の解消」のため寸法ラベルを選択時のみ表示に変更した経緯があり、B2(共有ページでの人物視認性改善)で同じ轍を踏まないよう注意が必要だった
- → `PersonIcon2D`の`variant='shared'`では、人物本体の各シェイプのopacity(0.85〜0.95)だけを上げて視認性を確保し、身長ラベルは小さいフォント・opacity 0.55・背景なしの「補足情報」程度に留めた。家具の寸法ラベルには一切触れていない
- 教訓: 「見えにくい」という指摘への対応でラベル(数値・テキスト)を強化するのは手軽だが、過去に「採寸図感をなくす」ために弱めたラベル類を再び強めると別の観点で後退する。視認性改善は対象物そのもの(色・不透明度・形状)の強化を優先し、ラベル強化は最終手段にする

**[設計パターン] 「課金が必要な本検証」への準備作業は、`not_run`/`demo`/`real`の3モードに分けて「未実施」を明示する**
- Keepa API検証の事前準備(課金・APIキー設定はまだ行わない)で、「既存のmockプレビュー機能を使って本検証したように見せてはいけない」という制約があった
- → `verify-keepa.ts`を3モード化: `KEEPA_API_KEY`未設定時はデフォルトで`not_run`(実APIを呼ばず全項目`false`+`NOT_RUN`エラー文言+`decision: 'not_run'`で出力、exit 0)、`--demo`/`KEEPA_DEMO=1`を明示指定した場合のみ旧mockプレビュー相当(`source: 'mock_demo'`、`[DEMO]`接頭辞、レポートに「本番採用判断には使用できません」の警告)を出力、`KEEPA_API_KEY`設定時は`real`(実API)。すべての出力に`source`/`mode`フィールドを持たせ、どのデータかを常に判別可能にした
- 教訓: 「本番実行はまだしない」という制約のあるタスクでは、(1)未実施を示す安全なデフォルト、(2)フォーマット確認用のダミーデータモード、(3)本番モード、を明確に3分割し、出力に必ず`source`/モード識別子を埋め込むと、後で見たときに「これは本物のデータか」が一目で分かる。テンプレート・出力先ディレクトリの動作確認は(1)のデフォルト実行だけで完結できる

**[設計パターン] 検証対象リストはコードから分離してJSON fixtureに置く**
- Keepa検証対象を20件→40件(10カテゴリ×4件)に拡張する際、`verify-keepa.ts`にハードコードされていた配列を`scripts/fixtures/keepa-targets.json`に切り出し、スクリプト側は`readFileSync`で読み込むだけにした
...
**[決定] 広告/アフィリエイト方針確定 — 初期公開時は導入しない（ドキュメント記録のみ・コード変更なし）**
- オーナーより「初期公開時は広告・アフィリエイトを入れない。Amazonアソシエイトも初期公開時点では導入しない。まず『おけるかな』が実際に使われるか、ビュー数・利用状況・共有数を確認し、伸びてきた段階で広告・アフィリエイトを再検討する。将来的な収益化候補としてAmazonアソシエイト・広告掲載はバックログに残す」との方針が確定
- `Business-Tracker.md`(「収益化方法検討」行・アイデアバックログに新規追加)・`furniture-layout-simulator/docs/owner-decision-pack.md`(現在の状態まとめ・オーナー回答欄No.10チェック・新設セクション9)・`furniture-layout-simulator/docs/release-checklist.md`(セクション13)に記録
- 広告タグ追加・Amazonアソシエイト申請/導入・UIへの広告枠追加は一切実施せず、`01_Threads_Automation`にも変更なし

**[完了] B3「LPの独自性改善」実装完了 — 「独自性」をビジュアル装飾ではなくコピー・構成の再定義として実現**
- 優先度B/Cバックログ記載時(2026-06-12)のB3は「ヒーロー以外にも部屋イラスト/アイコンを活用しブランドらしさを追加する」というビジュアル装飾路線だったが、オーナーからの実装指示では「LP全体を『SaaSっぽい機能紹介』から『家具を買う前の失敗を防ぐ生活者向けの体験』に寄せる」というコピー・構成面の再定義に変更された
- 主語を「ツール」から「買う前の不安」へ、「試す」より「置いてみる/買う前に確認する」を優先、という言葉選びの方針転換のみで、セクション構成・コンポーネント構造は変更せず7項目を実装(ヒーローバッジ・CTA文言統一・3ステップ書き換え・課題セクションのアクセントカラー・ダークセクション再構成・下部CTAのベネフィット化・モバイル見出し改行修正)
- 教訓: バックログ整理時に記載した「実装方針」は、実際に着手する際にオーナーから方向性が上書きされることがある。着手前に必ずオーナーの最新指示を優先し、バックログの記述は「過去の暫定方針」として扱う

**[再確認・3回目] `next dev`(オーファンプロセス)と`next build`(preflight内)を同じ`apps/web/.next`に対して同時実行すると、E2E用webServerの`next dev`が31/31全テスト同一箇所(`manual-product-button`/`simulator-mobile-fallback`要素未検出)で失敗する**
- B1(2026-06-12)・B2(2026-06-12)に続き3回目の発生。3ステップ見出し修正後にpreflightを再実行した際、直前に停止したdevサーバーのオーファンプロセスが`apps/web/.next`への書き込みを続けたまま`next build`が走り、ビルド成果物が破損。Playwright自身の`next dev -p 3000`webServerがその破損成果物を配信し、全テストが最初の`beforeEach`アサーションで一律失敗した
- 検出・復旧手順(再確認): `Get-Process node`でステイルなnode.exeを特定 → `netstat -ano | grep ':3000'`でLISTENING中のPIDを特定 → 該当PID全てを`Stop-Process -Force` → `rm -rf apps/web/.next` → `pnpm run preflight`再実行で解決(31/31 PASS)
- 教訓: 「type-check/buildはPASSするがE2Eが全件同一箇所で落ちる」パターンを見たら、実装ミスではなく`.next`破損を即疑い、上記手順を実行する。**この手順をCLAUDE.mdレベルの定型対応として昇格させる価値がある**(3回連続で同一パターン・同一解決法)

**[再確認] `TaskStop`はバックグラウンドの`pnpm --filter web run dev`配下の`node.exe`を確実には停止しない。ポート3000のPIDを`netstat`で個別に確認・`Stop-Process`する必要がある**
- 今回もTaskStop後にport 3000がLISTING状態のまま残るケースが複数回発生(スクリーンショット撮影後のdevサーバー含む)。`netstat -ano | grep 'LISTENING.*:3000'`での確認を都度行う運用を継続する

**[実装パターン] CJK見出しの不自然な改行は、`<br/>`追加ではなく自然な意味区切りでの文字数削減で解消する**
- 390px viewportで「買う前に確認する3つのステップ」(14文字)が「ステップ」の途中(「ステッ」/「プ」)で改行されるバグを、モバイルレビューで発見
- → 「3つの」→「3」に短縮(14→12文字)し、デスクトップでの単一行表示を維持したまま改行位置を解消。`<br/>`を追加すると意図しない箇所でデスクトップ表示も2行化するため、文字数そのものを減らす方が安全
- 教訓: ブラウザはCJK文字をどの文字境界でも改行できるため、ぎりぎりの幅の見出しは1文字の増減で改行位置が大きく変わる。モバイル幅での見出し確認は実装の最終ステップとして必須(B3でも今回含め複数回見出し改行の修正が発生している)

**[完了] `docs/owner-decision-pack.md`作成 — 公開判断を1ドキュメントに集約**
- B1-B3完了・preflight全PASS後、「4時間後にオーナーが公開判断を行う」ための準備として、Keepa案A/B・正式サービス名・運営者名・問い合わせ先・管轄裁判所・ドメイン・デプロイ先・GA4・広告/アフィリエイトの10項目を1つの判断テーブル+チェックボックス回答欄+案A/B別の実行手順にまとめた
